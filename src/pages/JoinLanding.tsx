...
const TOKENS = {
  ...
  shadows: {
    ...
    push: (color: string) => `0 4px 0 ${color}`,
  },
};
...
const normalizeDiscordInviteUrl = (rawUrl?: string) => {
  if (!rawUrl) {
    return "";
  }
...
type ButtonVariant = "primary" | "discord" | "ghost" | "white";
const Button = ({
  children,
  variant = "primary",
  className = "",
  ...
  if (href) {
    return (
      <a
        href={href}
        className={`${baseStyle} ${variants[variant as ButtonVariant]} ${className}`}
        style={shadowStyle}
        {...props}
      >
        {children}
...
const DemoInviteToast = ({ isVisible, onClose }: { isVisible: boolean; onClose: () => void }) => (
  <AnimatePresence>
    {isVisible && (
      <motion.aside
        role="status"
...
const Hero = () => {
  const [activeHeroPhoto, setActiveHeroPhoto] = useState<string | null>(null);
  ...
  const handleHeroPointerDown = (id: string) => (event: React.PointerEvent) => {
  ...
  const handleHeroBackgroundPointerDown = (event: React.PointerEvent) => {
    if (event.pointerType === "mouse") {
      return;
    }
  ...
  const getHeroZIndex = (id: string, base: string) => (activeHeroPhoto === id ? "z-40" : base);
...
const CTA = ({ discordInviteProps }: { discordInviteProps: Record<string, unknown> }) => {
  const reduceMotion = useReducedMotion();
  const revealProps = reduceMotion
    ? { initial: false, whileInView: { opacity: 1, y: 0 } }
    : {
...
export default function JoinLanding() {
  ...
  const showDemoInviteToast = (event?: { preventDefault?: () => void }) => {
